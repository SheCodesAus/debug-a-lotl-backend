        ...
        if not club.is_public:
            if not request.user.is_authenticated:
                return Response(
                    ...
                    status=status.HTTP_401_UNAUTHORIZED,  # fixed typo: UNATHORIZED
                )
...
# ↓ These were accidentally nested inside ClubJoinView — now fixed
    ...
    def get(self, request, pk):
        ...
        if request.user != club.owner:
            return Response(
                ...
                status=status.HTTP_403_FORBIDDEN,  # fixed typo: FORBIDENN
            )
...
class MemberStatusUpdateView(APIView):
    ...
    def patch(self, request, club_pk, member_pk):
        ...
        if new_status not in [Member.STATUS_APPROVED, Member.STATUS_REJECTED]:
            return Response(
                {"detail": "Status must be approved or rejected"},
                status=status.HTTP_400_BAD_REQUEST,  # fixed typo: HTPP_400
            )
...
class MeetingListCreate(APIView):
    ...
    def post(self, request, club_id):
        ...
        if club.owner != request.user:
            return Response(
                {"detail": "Only the owner can create meetings."},
                status=status.HTTP_403_FORBIDDEN,
            )
